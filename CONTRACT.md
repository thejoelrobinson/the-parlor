...
```js
{
  ...
  viewFor(state, side) -> view,        // JSON-safe view for one side; may hide information
  aiMove(state, side) -> Move,         // must ALWAYS return a legal move for side; must terminate
...
6. AI: always terminates, always legal. Keep search depth bounded; forced-capture sequences (checkers) are explored inside the search, not by unbounded external `applyMove` loops.
